...
-   $N(x)$...the standard normal cumulative distribution function;
    $r$...annualized continuously compounded risk-free rate; $\sigma$...
    annualized constant volatility; $S$...stock price; $K$...strike
    price; $T$...time to expiration for the option (in years)
...
::: center
![image](images/08-2-nx.png){width="3.5in"}
:::
:::
...
-   $S_0 e^{rT} N(d_1)$...The expected stock price at maturity in a
    risk-neutral world when stock prices less than the strike price are
    counted as zero.
...
-   $S_0 e^{rT} N(d_1) / N(d_2)$...Expected stock price in a
    risk-neutral world if option is exercised
...
-   The variable $\mu$ does not appear in the Black-Scholes-Merton
...
-   As $S_0$ becomes very large $c$ tends to $S_0 - Ke^{-rT}$ because
    $d_1$ and $d_2$ become very large, and $N(d_1)$ and $N(d_2)$ become
    close to 1.

-   On the other hand, $p$ tends to zero because $N(-d_1)$ and
    $N(- d_2)$ become close to 0.

-   As $S_0$ becomes very small $c$ tends to zero and $p$ tends to
    $Ke^{-rT} - S_0$
:::
...
-   The standard deviation of the return in a short time period
    $\Delta t$ is approx. $\sigma \sqrt{\Delta t}$

-   If a stock price is USD 30 and its volatility is 25% per year what
    is the standard deviation of the price change in one day?
    $$\sigma \sqrt{\Delta
                          t} = 30 \times \sqrt{\frac{1}{252}} = 1.89$$

-   On the other hand, when you calculate volatility as a standard
    deviation of daily returns, you can multiply the result by
    $\sqrt{252}$ to obtain annulized volatility.
:::
...
-   For this reason time is usually measured in [trading days]{.alert}
...
-   Unlike the price of the underlying, volatility is not an observable
...
-   Volatility can also be inferred from option prices. This inferred
    volatility is called the [implied volatility]{.alert}.
...
![image](images/08-3-vix.png){width="3.3in"}
...
-   Implied volatility can be used to assess the relative value of
    different options, neutralizing the effects of moneyness and time to
    expiration (low implied volatility = cheap options)

-   Implied volatility is useful for revaluing existing positions over
    time.
...
-   Although historical volatility can be estimated, there is no
    objective measure of future volatility.

-   Implied volatility is the BSM model volatility that yields the
    market option price.

-   Implied volatility is a measure of future volatility, whereas
    historical volatility is a measure of past volatility.

-   Option prices reflect the beliefs of option market participants
    about the future volatility of the underlying.

-   The volatility smile is a two dimensional plot of the implied
    volatility with respect to the exercise price.

-   The volatility surface is a three-dimensional plot of the implied
    volatility with respect to both expiration time and exercise prices.

-   If the BSM model assumptions were true, then one would expect to
    find the volatility surface flat, but in practice, the volatility
    surface is not flat.
:::
...
-   Early exercise is sometimes optimal at time $t_i$ if the dividend at
    that time is greater than $K[1 - e^{-r (t_{i+1} - t_i)}]$
:::
...
1.  The 1st European price is for an option maturing at the same time as
    the American option
